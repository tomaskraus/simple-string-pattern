...
const simple_string_pattern_1 = __importDefault(
  require('#src/simple-string-pattern')
);
describe('Full Pattern: constructor errors', () => {
  ...
  test('Does not accept strings that contain unescaped control chars.', () => {
    ...
    expect(
      () => new simple_string_pattern_1.default(unescapedMultilineStr)
    ).toThrow();
  });
...
describe('Full Pattern: constructor and getters - multiple characters', () => {
  ...
  test('If the leading double-quote is escaped, does not create an Exact Pattern.', () => {
    expect(new simple_string_pattern_1.default('"hello"').value()).toEqual(
      '"hello"'
    );
    expect(new simple_string_pattern_1.default('\\"hello"').value()).toEqual(
      '\\"hello"'
...
describe('Partial Pattern: constructor errors', () => {
  ...
  test('Does not accept strings that contain unescaped control chars.', () => {
    const tabStr = String.fromCharCode(9);
    expect(
      () => new simple_string_pattern_1.default('... ' + tabStr)
    ).toThrow();
    const unescapedMultilineStr = 'a\nb';
    expect(
      () => new simple_string_pattern_1.default(unescapedMultilineStr + ' ...')
  ...
  test('Does not accept a string its pattern body ends with odd number of consecutive backslash characters.', () => {
    expect(() => new simple_string_pattern_1.default('... \\ ...')).toThrow();
    expect(
      () => new simple_string_pattern_1.default('... \\\\\\ ...')
    ).toThrow();
    expect(
      () => new simple_string_pattern_1.default('... a\\\\\\ ...')
    ).toThrow();
  });
});
// ----------------------------------------------------------------------------
describe('Partial Pattern: constructor and getters - single character', () => {
  test('Does allow one character (ascii)', () => {
    expect(new simple_string_pattern_1.default('... a').value()).toEqual(
      '... a'
    );
  });
  test('Does allow one character (non-ascii)', () => {
    expect(new simple_string_pattern_1.default('き ...').value()).toEqual(
      'き ...'
    );
  });
  test('Does allow one character (emoji)', () => {
...
describe('Full Pattern: constructor and getters - escape sequences', () => {
  ...
  test("If the pattern body's leading double-quote is escaped, does not create an Exact Partial Pattern.", () => {
    ...
    expect(
      new simple_string_pattern_1.default('... \\"hello"').value()
    ).toEqual('... \\"hello"');
  });
});
